...
class Authenticator
{
    ...
    public function logoutUser(): bool
    {
        ...
        $logged_out = Tempstore::delete('default_current_user');
        if($logged_out) {
            session_destroy();
            return true;
        }
    ...
    public function verificationToken(int $uid): string|false
    {
        ...
        if($user) {
            ...
            if($database->getDatabaseType() == 'sqlite') {
                $query = "CREATE TABLE IF NOT EXISTS `verification_tokens` (vid INTEGER primary key autoincrement, token varchar(255), uid INTEGER NOT NULL, created_at INTEGER NOT NULL)";
            }
