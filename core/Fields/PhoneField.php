...
class PhoneField implements FieldInterface
{
    ...
    public function markUp(array $field_value): string
    {
        ...
        foreach ($field_value as $value) {
            ...
            if($display_name === 'tel_to') {
                $field_value = "<a href='mailto:".$value['value']."' title='send mail to ".$value['value']."'>".$value['value']."</a>";
            }
            if($display_name === 'on_image') {
                $field_value = "<img src='".$this->phoneToImage(trim($value['value']))."' alt='image of email' />";
            }
        }

        return Services::create('render')->render('phone_field_display_markup.php',['value' => $field_value, 'setting' => $setting]);
    }

    private function phoneToImage(string $text): string
    {
