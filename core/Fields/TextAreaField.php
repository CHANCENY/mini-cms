    ...
    public function markUp(array $field_value): string
    {
        ...
        foreach ($field_value as $value) {
            if($display_name === 'trimmed') {
                $field_value = trim(substr(reset($value), 0, 255));
            }
            if($display_name === 'full_html') {
                $field_value = html_entity_decode(reset($value));
            }
            if($display_name === 'full_text') {
                $field_value = trim(reset($value));
            }
            if($display_name === 'plain_text') {
                $field_value = htmlspecialchars(strip_tags(html_entity_decode(reset($value))));
            }
        }
