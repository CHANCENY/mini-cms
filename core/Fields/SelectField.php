...
class SelectField implements FieldInterface
{
    ...
    {
        return $this->field['field_type'] ?? 'select';
    }
    ...
    public function save(): bool
    {
        ...
        $size = (string) $this->field['field_settings']['field_size'] ?? 255;
        if(strpos($size, ',')) {
            $list = explode(',', $size);
            $size = $list[0] ?? 255;
            $this->field['field_settings']['field_is_multiple'] = end($list);
            $this->field['field_settings']['field_size'] = $list[0] ?? 255;
        }
        else {
            $this->field['field_settings']['is_multiple'] = null;
        }
        if(!empty($fields)) {
            return false;
        }
    ...
    public function markUp(array $field_value): string
    {
        ...
        foreach ($field_value as $value) {
            if($display_name === 'ordered_list' || $display_name === 'unordered_list') {
                $list .= "<li>". trim($value['value'])."</li>\n";
            }
            if($display_name === 'text') {
                $list .= "<p>". trim($value['value']). "</p>\n";
            }
        }
        if($display_name == 'ordered_list') {
            $field_value = "<ol>".$list."</ol>";
        }
        if($display_name == 'unordered_list') {
            $field_value = "<ul>".$list."</ul>";
        }
        if($display_name == 'text') {
            $field_value = "<div class='field-value-wrapper'>".$list."</div>";
        }
        return Services::create('render')->render('select_field_display_markup.php',['value' => $field_value, 'setting' => $setting]);
    }
    ...
    {
       return !empty($this->field['field_settings']['field_is_multiple']);
    }
}